...
public interface TransactionRepository extends JpaRepository<Transaction,Integer> {
    ...
    @Query(value = "select * from transaction t join users u on t.user_id=u.id where t.user_id = :id order by t.date DESC limit 5",nativeQuery = true)
    List<Transaction> getTransactionsHistory(@Param("id") Long id);

    @Query(value = "select count(amount) from transaction where type = 'transfer'",nativeQuery = true)
    Double getCompleteTransactions();

    @Query(value = "select sum(amount) from transaction where type = 'transfer'",nativeQuery = true)
    Double getCompleteTransactionsAmount();
}
